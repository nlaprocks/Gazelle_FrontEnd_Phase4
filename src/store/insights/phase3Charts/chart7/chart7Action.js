...
import { CHART7_REQUEST, CHART7_SUCCESS, CHART7_FAILURE } from "./chart7Type";
...
const fetchChart7Data = ({
  ...
  return async (dispatch) => {
    try {
      dispatch(chart7Request(loading));
      let api = `${process.env.REACT_APP_NGROK}/insights/chart7?project_id=${encodeURIComponent(project_id)}&model_id=${encodeURIComponent(model_id)}`;
      ...
      const { data } = res;
      loading = false;
      dispatch(chart7Success(data));
    } catch (error) {
      const msg = error.response;
      loading = false;
      dispatch(chart7Failure(msg));
      console.log(error);
    }
