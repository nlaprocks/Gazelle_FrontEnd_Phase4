...
import { CHART5_REQUEST, CHART5_SUCCESS, CHART5_FAILURE } from "./chart5Type";
...
const fetchChart5Data = ({
  ...
  return async (dispatch) => {
    try {
      dispatch(chart5Request(loading));
      let api = `${process.env.REACT_APP_NGROK}/insights/chart5?project_id=${encodeURIComponent(project_id)}&model_id=${encodeURIComponent(model_id)}`;
      ...
      const { data } = res;
      loading = false;
      dispatch(chart5Success(data));
    } catch (error) {
      const msg = error.response;
      loading = false;
      dispatch(chart5Failure(msg));
      console.log(error);
    }    
