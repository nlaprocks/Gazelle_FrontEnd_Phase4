...
export default function PromoEventSimulator({
  ...
  const getApexSeries = (data) => {
    ...
    return data?.data?.datasets?.map((dataset) => ({
      name: dataset.label,
  ...
  //   if (!data || !data.chartData || !data.chartData.datasets) {
  //     return []; // Return an empty array if datasets are not available
  //   }

  //   return data.chartData.datasets.map((dataset) => ({
  //     name: dataset.label,
  //     data: dataset.data,
  //     type: dataset.type || 'bar',  // Can be dynamically assigned
  //   }));
  // };
