...
const UpdateProject = ({
  ...
  const path =
    project?.company_logo !== null
      ? Object.keys(project?.company_logo).length !== 0
        ? project?.company_logo?.split(
  ...
  const updateProjectReducer = useSelector(
    (state) => state.updateProjectReducer
  );
  const handleClose = () => {
  ...
  const updateProject = () => {
    ...
    dispatch(allActions.updateProjectAction.updateProject(formData));
  };
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (updateProjectReducer.success) {
      dispatch(allActions.getAllProjectsAction.getAllProjects());
      handleClose();
      delete updateProjectReducer.success;
    }
  }, [updateProjectReducer]);
  return (
    <Modal show={showUpdateProject} onHide={handleClose} centered scrollable>
      <Modal.Header>
        <Modal.Title>
