...
const generateCommonUIElements = (slide, value, pptx) => {
  ...
  slide.addText(
    value?.SlideNames[0]?.slide_title ? value?.SlideNames[0]?.slide_title : "",
    {
      x: 0.2,
      y: 1,
      w: 12,
...
const PptDownloader = ({ value, id, role }) => {
  ...
  const RunDemo = async (value, id, role) => {
      ...
      if (role === "users") {
        function transformChartData(inputData) {
          inputData.Charts.forEach((data, index) => {
            ...
            slide.addChart(
              pptx.ChartType[data?.chart_type === "bar" ? "bar" : "line"],
              transformedData,
              chartProps
            );
          });
        ...
        if (id === 0) {
          ...
          productData.forEach((item) => {
            ...
            productChartData[product].dollarSales.push(
              item.Dollar_sales_last_52_weeks
            );
            productChartData[product].priceElasticity.push(
              item.Base_Price_Elasticity
            );
          });
          ...
          chart6Data.forEach((item) => {
            ...
            retailers[retailer].Promo_Price_Elasticity.push(
              item.Promo_Price_Elasticity
            );
            retailers[retailer].Average_discount_depth.push(
              item.Average_discount_depth
            );
          });
          ...
          Object.keys(retailers).forEach((retailer, index) => {
            ...
            const chartProps = {
              ...
              catAxes: [
                { catAxisTitle: "Change", catAxisTickLabelPosition: "high" },
                { catAxisHidden: true },
              ],
            };
          ...
          Object.keys(retailers).forEach((retailer, index) => {
            ...
            const chartDataArray = [
              {
                type: pptx.charts.BAR,
                data: retailerData.data.datasets.map(
