...
const PptDownloader = ({ value, id, role }) => {
    // loader
    ...
    const RunDemo = async (value, id, role, type = null) => {
        console.log("value::: ", value);
        console.log("id::: ", id);
        console.log("role::: ", role);
        // type needed for brand wise and retailer wise for chart 1 right now
        ...
        try {
            ...
            // **chart 1
            if (role === "users") {
                function transformChartData(inputData) {
                    inputData.Charts.forEach((data, index) => {
                        ...
                        transformedData = transformedData.concat(chartData);
                        if (index > 0) {
                            pptx.layout = "LAYOUT_WIDE";
                            pptx.defineSlideMaster({
                                title: "PLACEHOLDER_SLIDE",
                        ...
                        const chartProps = {
                            ...
                            catAxisTitle: data.x_axis_label,
                            showLegend: true,
                            legendPos: "t",
                            valAxisTitle: data.y_axis_label,
                            showCatAxisTitle: true,
                        };
                        slide = pptx.slides[index];
                        generateCommonUIElements(slide, value, pptx);
                        slide.addChart(pptx.ChartType[data?.chart_type === "bar" ? "bar" : "line"], transformedData, chartProps);
                    });
                }
                ...
                if (id === 0) {
                    if (type === "Brand") {
                        ...
                        chart1Reducer?.chart1Data?.data.forEach((item) => {
                            ...
                            groupedData[brand][retailer].labels.push(product);
                            groupedData[brand][retailer].values.push(price);
                        });
          ...
          formattedChartData.forEach((retailer, index) => {
            ...
            slide.addChart(pptx.charts.LINE, datasets, chartOptions);
                    ...
                    Object.keys(productRetailerMap).forEach((ddd, index) => {
                        ...
                        const volumeValues = uniqueChanges.map((change) => {
                            ...
                            return value !== null ? parseFloat(value.toFixed(2)) : null; // Rounding to 2 decimal places
            ...
            await pptx.writeFile({ fileName: `${value.question}.pptx` });
        } catch (e) {
            // console.error("e::: ", e);
    ...
    return (
        <>
            {/* if id is 0 then it means chart 1.  */}
            {id === 0 ? (
                        ...
                        <ul className="dropdown-menu show">
                            <li>
                                <a className="dropdown-item" href="#!" onClick={(e) => {
                                    e.stopPropagation();
                                    RunDemo(value, id, role, "Brand");
                                }}>Brand Wise</a>
                            </li>
                            <li>
                                <a className="dropdown-item" href="#!" onClick={(e) => {
                                    e.stopPropagation();
                                    RunDemo(value, id, role, "Retailer");
                                }}>Retailer Wise</a>
                            </li>
                        </ul>
                    ) : null}
