...
const AllPptDownloader = () => {
            ...
            if (adminQuestionsReducer?.question?.data && adminQuestionsReducer?.question?.data[8]) {
                ...
                transformedChartData.forEach((retailerData, index) => {
                    ...
                    const chartProps = {
                        ...
                        showGridlines: false,
                        catAxisTitle: retailerData.Retailer,
    ...
    const RunDemo = async () => {
        try {
            ...
            await generateSecondSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateThirdSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateFourthSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateFifthSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateSixthSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateSeventhSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateEightSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
            await generateNingthSlide(pptx.addSlide({ masterName: "PLACEHOLDER_SLIDE" }), pptx);
