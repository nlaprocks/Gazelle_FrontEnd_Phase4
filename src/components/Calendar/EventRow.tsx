...
const EventRow: React.FC<EventRowProps> = ({
  ...
  const calculateEventPosition = (event: Event) => {
    const startWeek = Math.max(0, differenceInWeeks(startOfWeek(event.startDate), yearStart));
    const endWeek = Math.min(51, differenceInWeeks(startOfWeek(event.endDate), yearStart));
  ...
  const isEventVisible = (event: Event) => {
    ...
    return !isBefore(event.endDate, yearStart) && !isAfter(event.startDate, yearEnd);
  };
